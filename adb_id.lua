...
function adbOnItemIdStatsLine(name, line, wildcards, styles)
    ...
    if last_enchant ~= "" then
        ...
        if stat ~= nil then
            ...
            idObject.enchants[adb_enchant_stats[stat]] =
                adbGetStatNumberSafe(idObject.enchants[adb_enchant_stats[stat]]) + tonumber(value)
        end
    end
...
function inv.items.trigger.itemIdStats(line)
    ...
    local isPartialId, id, name, level, weight, wearable, score, keywords, itemType, worth, flags, affectMods,
        continuation, material, foundAt, ownedBy, clan, rawMaterial
    ...
    local acid, cold, energy, holy, electric, negative, shadow, magic, air, earth, fire, light, mental, sonic, water,
        poison, disease
    ...
    if (continuation ~= nil) then
        ...
        if (flagsContinuation) then
            ...
            inv.items.setStatField(inv.items.identifyPkg.objId, invStatFieldFlags, (inv.items
                .getStatField(inv.items.identifyPkg.objId, invStatFieldFlags) or "") .. " " .. continuation)
        ...
        elseif (affectModsContinuation) then
            ...
            inv.items.setStatField(inv.items.identifyPkg.objId, invStatFieldAffectMods, (inv.items
                .getStatField(inv.items.identifyPkg.objId, invStatFieldAffectMods) or "") .. " " .. continuation)
        ...
        elseif (keywordsContinuation) then
            ...
            inv.items.setStatField(inv.items.identifyPkg.objId, invStatFieldKeywords, (inv.items
                .getStatField(inv.items.identifyPkg.objId, invStatFieldKeywords) or "") .. " " .. continuation)

        elseif (nameContinuation) then
            -- Add the continuation to the existing name
            inv.items.setStatField(inv.items.identifyPkg.objId, invStatFieldName, (inv.items
                .getStatField(inv.items.identifyPkg.objId, invStatFieldName) or "") .. " " .. continuation)
    ...
    if (weightReduction ~= nil) then
        inv.items.setStatField(inv.items.identifyPkg.objId, invStatFieldWeightReduction, dbot.tonumber(weightReduction))
